...
<body class="font-sans">
        ...
        <aside class="sidebar-desktop w-64 main-gradient text-white flex flex-col shadow-2xl">
            ...
            <nav class="flex-1 mt-4 space-y-2" id="sidebar-nav">
                ...
                <a href="#" data-page="assign" class="flex items-center py-3 px-4 text-sites-light font-semibold hover:bg-white/10 rounded-lg transition-colors nav-link mx-3">
                    <i data-lucide="file-text" class="w-5 h-5 mr-3"></i>Task
                </a>
    ...
    <button 
        class="w-full flex items-center py-3 px-4 text-sites-light font-semibold text-left hover:bg-white/10 rounded-lg transition-colors"
                ...
                <section id="dashboard-content" class="content-page active">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-1 space-y-8">
                            <div id="meeting-card" class="w-full text-left bg-white text-gray-800 rounded-xl shadow-lg p-6 border-t-4 border-sites-accent transition-all">
                                ...
                                <a id="meeting-link" href="#" class="text-sites-accent font-semibold hover:underline break-words" target="_blank" rel="noopener noreferrer">https://meet.google.</a>
                            </div>
                 ...
                 <section id="assign-content" class="content-page">
                    <!-- This content is now generated by renderAssignPage() in JavaScript -->
                </section>
    ...
    <!-- Task Progress Modal -->
    <div id="task-progress-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'task-progress-modal')">
        <div class="modal-container bg-white rounded-xl shadow-2xl max-w-lg w-full flex flex-col relative">
            <header class="p-6 border-b flex justify-between items-start">
                 <div>
                    <h2 id="progress-task-title" class="text-2xl font-bold text-gray-800">Task Name Here</h2>
                    <p id="progress-task-date" class="text-sm text-gray-500 font-medium">Due: YYYY-MM-DD</p>
                 </div>
                 <div class="flex items-center space-x-2">
                    <button id="progress-edit-btn" class="text-gray-400 hover:text-sites-accent"><i data-lucide="edit-3" class="w-5 h-5"></i></button>
                    <button onclick="hideTaskProgressModal()" class="text-gray-400 hover:text-gray-700"><i data-lucide="x" class="w-6 h-6"></i></button>
                 </div>
            </header>
            <div class="p-6 space-y-5">
                <div>
                    <div class="flex justify-between mb-1">
                        <span class="text-base font-medium text-sites-dark">Progress</span>
                        <span id="progress-bar-text" class="text-sm font-medium text-sites-dark">0%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-4">
                        <div id="progress-bar-fill" class="bg-sites-accent h-4 rounded-full transition-all duration-300" style="width: 0%"></div>
                    </div>
                </div>
                <div id="progress-subtask-list" class="space-y-3 max-h-60 overflow-y-auto pr-2">
                    <!-- Subtask checkboxes will be rendered here -->
                </div>
            </div>
            <footer class="p-4 border-t bg-gray-50 rounded-b-xl">
                <button id="progress-submit-btn" class="w-full bg-sites-accent text-white font-bold py-2 rounded-lg hover:opacity-90 transition-opacity">Submit</button>
                <button id="progress-unsubmit-btn" class="w-full bg-red-600 text-white font-bold py-2 rounded-lg hover:bg-red-700 transition-opacity hidden">Un-submit</button>
            </footer>
        </div>
    </div>
    
    <!-- Other Modals (wrapped in overlays for consistent closing behavior) -->
    <div id="task-details-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'task-details-modal')">
        <div class="modal-container bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col relative">
    ...
    <div id="meeting-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'meeting-modal')">
        <div class="modal-container bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 relative">
    ...
    <div id="calendar-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'calendar-modal')">
        <div class="modal-container bg-white rounded-xl shadow-2xl max-w-4xl w-full h-auto flex flex-col p-4 relative">
    ...
    <div id="add-task-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'add-task-modal')">
        <div class="modal-container bg-modal-bg rounded-xl shadow-2xl max-w-xl w-full flex flex-col relative text-sites-dark">
    ...
    <!-- Edit Task Modal -->
    <div id="edit-task-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'edit-task-modal')">
        <div class="modal-container bg-modal-bg rounded-xl shadow-2xl max-w-xl w-full flex flex-col relative text-sites-dark">
            <header class="p-6 border-b border-sites-dark/10">
    ...
    <div id="delete-task-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'delete-task-modal')">
        <div class="modal-container bg-white rounded-lg shadow-xl p-6 max-w-sm w-full text-center">
    ...
    <div id="member-details-modal" class="modal-overlay fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 hidden" onclick="closeModal(event, 'member-details-modal')">
        <div class="modal-container bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col relative">
